...
def word_similarity(source_text,suspicious_text):
    import nltk
    #nltk.download('punkt')
    tokenized_suspicious_text = suspicious_text.lower().split()
    tokenized_source_text = source_text.lower().split()
    ...
    #print(punctuation)
    cleaned_tokenized_suspicious_text = [token for token in tokenized_suspicious_text if token not in stopwords and token not in punctuation]

    cleaned_tokenized_source_text = [token for token in tokenized_source_text if token not in stopwords and token not in punctuation]
